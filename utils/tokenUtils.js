...
const verifyToken = (token, tokenType = "access") => {
  try {
    ...
    // Remove "Bearer ", whitespace, and surrounding quotes/backticks
    ...
    let secret = tokenType === "access" ? process.env.JWT_ACCESS_SECRET : 
                 tokenType === "refresh" ? process.env.JWT_REFRESH_SECRET : 
...
const setAuthCookies = (res, accessToken, refreshToken) => {
  const cookieOptions = {
    httpOnly: true,
    // Must be true if using sameSite: 'none' (common for cross-domain API/Frontend)
    secure: isProduction || true, 
    sameSite: isProduction ? "none" : "lax",
    maxAge: 24 * 60 * 60 * 1000, // 1 day for access token
  };

  // Crucial for inheritance across subdomains
  if (isProduction) {
    cookieOptions.domain = ".rehabmedico.in";
  }
