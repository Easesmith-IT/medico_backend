...
const socialPostSchema = new mongoose.Schema(
  {
    ...
    type: {
      type: String,
      enum: ['TEXT', 'GALLERY', 'REEL', 'ARTICLE'],
      required: true
    },
    ...
    // ======================
    // FOLLOW (embedded - optional design)
    // ======================
    follows: [
      {
        followerId: {
...
// Visibility filtering (VERY IMPORTANT)
socialPostSchema.index({ isHidden: 1, createdAt: -1 });

// Hashtag search (fast filtering)
socialPostSchema.index({ hashtags: 1 });

// Mentions lookup
socialPostSchema.index({ mentions: 1 });

// Like & comment queries
socialPostSchema.index({ 'likes.userId': 1, createdAt: -1 });
socialPostSchema.index({ 'comments.userId': 1, createdAt: -1 });

// Follow queries
socialPostSchema.index({ 'follows.followerId': 1 });
socialPostSchema.index({ 'follows.followingId': 1 });

// Engagement sorting (feeds, trending)
